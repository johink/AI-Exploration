...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        """
          Returns max_value Q(state,action)
    ...
    def computeActionFromQValues(self, state):
        ...
        if not actions:
            return None
        action_values = [(action, self.getQValue(state, action)) for action in actions]
    ...
    def update(self, state, action, nextState, reward):
        ...
        self.qvalues[qstate] = (1-self.alpha) * self.getQValue(*qstate) + self.alpha * sample
