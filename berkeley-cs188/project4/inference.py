...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for pos in self.legalPositions:
                # p(true pos | noisy pos) =
                # p(noisy pos | true pos) * p(true pos) / p(noisy pos)
                # sum(p(noisy pos | pos_i) * p(true pos)) for all possible true pos
                # true pos = possible hypothesis (position)

                trueDistance = util.manhattanDistance(pos, pacmanPosition)
                prior = beliefDist[pos]
                likelihood = emissionModel[trueDistance]

                if emissionModel[trueDistance] > 0:
                    allPossible[pos] = likelihood * prior
    ...
    def elapseTime(self, gameState):
        ...
        newPosDist[p] = Pr( ghost is at position p at time t + 1 | ghost is at
